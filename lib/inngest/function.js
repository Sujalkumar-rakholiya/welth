    ...
    async ({ step }) => {
        ...
        for (const budget of budgets) {
            ...
            await step.run(`check-budget-${budget.id}`, async () => {
                ...
                if (
                    percentageUsed >= 80 && (!budget.lastAlertSent || isNewMonth(new Date(budget.lastAlertSent), new Date()))
                ) {
                    //Send Email
                    await sendEmail({
    ...
    async ({ step }) => {
        ...
        for (const user of users) {
            await step.run(`generate-reports-${user.id}`, async () => {
                ...
                const insights = await generateFinsncialInsights(stats, monthName);
...
async function generateFinsncialInsights(stats, month) { }
